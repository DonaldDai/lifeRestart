...
class App{
    ...
    initPages() {
          ...
          this.#life.talentRandom().forEach((talent) => {
            ...
            li.click(() => {
              if (li.hasClass("selected")) {
                ...
                if (this.#talentSelected.size < initTalentCount) {
                  talentPage.find("#next").text(`请选择${initTalentCount}个`);
                }
              } else {
                if (this.#talentSelected.size == initTalentCount) {
                  this.hint(`只能选${initTalentCount}个天赋`);
                  return;
                }
        ...
        talentPage.find("#next").click(() => {
          if (this.#talentSelected.size != initTalentCount) {
            this.hint(`请选择${initTalentCount}个天赋`);
            return;
          }
          this.#totalMax =
            20 +
        ...
        function handleNext() {
            const trajectory = this.#life.next();
                ...
                if(isEnd) {
                    ...
                    trajectoryPage.find('#to_end').hide();
                } else {
                    // 如未死亡，更新数值
        ...
        trajectoryPage.find('#to_end').click(() => {
            const interval = 4e2;
            clearInterval(this.autoTimeId);
            this.autoTimeId = setInterval(() => {
                if (!this.#isEnd) {
